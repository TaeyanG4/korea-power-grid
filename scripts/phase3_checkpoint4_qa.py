...
EXTENSION_START = "2016-08"
EXTENSION_END = "2018-07"
FULL_START = "2016-08"
FULL_END = "2026-07"
EXPECTED_EXTENSION_RECORDS = 72
EXPECTED_FULL_RECORDS = 360
...
def read_manifests(root: Path) -> list[dict]:
    return [load_json(path) for path in sorted(root.rglob("manifest.json"))]
...
def allowed_status_accounting(status: Counter[str], expected: int) -> bool:
    return (
        sum(status.values()) == expected
        and not (set(status) - {"success", "source_unavailable"})
    )


def main() -> int:
    previous_checkpoint = load_json(
        ROOT / "data" / "audits" / "phase3_checkpoint3_summary.json"
    )
    download_qa = load_json(
        ROOT / "data" / "audits" / "checkpoint_2016-08_2018-07_qa.json"
    )
    format_qa = load_json(
        ROOT / "data" / "audits" / "format_qa_2016-08_2018-07.json"
    )
    ...
    extension_summary = load_json(
        ROOT / "data" / "processed" / "checkpoint_10y_extension" / "summary.json"
    )
    ...
    hard_checks = {
        "checkpoint3_passed": previous_checkpoint.get("status")
        == "PASS_WITH_OBSERVATIONS",
        "extension_download_records_72": int(download_qa["manifest_records"])
        ...
        "extension_format_qa_records_accounted_72": int(format_qa["records_accounted"])
        == EXPECTED_EXTENSION_RECORDS,
        "extension_format_qa_problem_count_zero": int(format_qa["problem_count"]) == 0,
        "extension_content_qa_records_72": int(extension_content_qa["records"])
        == EXPECTED_EXTENSION_RECORDS,
        ...
        "extension_processed_summary_records_72": int(extension_summary["records"])
        == EXPECTED_EXTENSION_RECORDS,
        "extension_processed_manifest_records_72": len(extension_manifests)
        == EXPECTED_EXTENSION_RECORDS,
        ...
        "combined_manifest_records_360": len(all_manifests) == EXPECTED_FULL_RECORDS,
    ...
    report = {
        ...
        "period": {"start": FULL_START, "end": FULL_END, "months": 120},
        "extension_period": {
    ...
    out = ROOT / "data" / "audits" / "phase3_checkpoint4_summary.json"
